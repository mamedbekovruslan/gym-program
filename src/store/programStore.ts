...
import { Exercise, Program, SessionState } from '../types/training';
...
export const useProgramStore = create<StoreState>()(
  persist(
    (set, get) => ({
      program: initialProgram,
      session: makeSession(),
      setProgramName: (name) =>
        set((state) => ({ program: { ...state.program, name } })),
      ...
      updateExercise: (id, updates) =>
        set((state) => ({
          program: {
            ...state.program,
            exercises: state.program.exercises.map((exercise) =>
      ...
      startSession: () =>
        set((state) => {
          if (state.program.exercises.length === 0) return state;
          return {
            session: {
              phase: 'work',
              startedAt: Date.now(),
          ...
          if (moreSets) {
            return {
              session: {
                ...
                restEndsAt:
                  currentExercise.restSec > 0 ? now + restDurationMs : null
              }
            };
          }
